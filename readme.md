...
- **本紙テンプレ**: `templates_src/申請書増車1.xlsx` または `templates_src/申請書減車1.xlsx`（出力ブックのシート1）。
...
- 仕様: 「増車 or 減車」の**同時アップロードは不可**（サーバでエラー返却）。未選択もエラー。
...
### 3.2 作成タブ
- **① 連絡書（増車）枠** / **② 連絡書（減車）枠**
  - D&D またはクリック選択。**複数ファイル**可（ただし「同時アップロード不可」のサーバ仕様で**片側のみ**有効）。
  - ファイル選択後は、**Excelアイコン（正方）**を右側に並べて表示。各アイコン右上の**×**で個別削除。
  - 「**ファイルを追加する**」ボタンは**1件以上選択時のみ表示**。0件に戻ると非表示。
- **③ 種別**（ラジオ）  
  **普通 / 小型 / けん引 / 被けん引**
- **④ 積載トン数**（セレクト）  
  - **増車のみ or 減車のみ**アップロード時: 単一セレクト `inc_weight_class` を表示。  
    `dec_weight_class` は hidden で `inc_weight_class` に同期（サーバ側は未指定時も inc を流用）。
  - 選択肢（UI値 → ラベル）  
    - `to_2t` → 2 . 0 トンまで  
    - `2t_long` → 2 . 0 トンロング  
    - `over_2t_long_to_7_5t` → 2 . 0 トンロング超～ 7 . 5 トンまで  
    - `over_7_5t` → 7.5トンを超えるもの
- **実行ボタン**
  - 送信前に .xlsx のZipシグネチャを簡易検証。
  - `FormData(form)` をそのまま送信（**重複appendなし**）。
  - **成功ダウンロード直後**に、選択ファイル・アイコン・件数・追加ボタン表示を**自動クリア**（失敗時は保持）。
...
- **同時アップロード禁止**: `increase_files` と `decrease_files` の両方が非空 → **400**（アラート）。
...
- 先頭に**シート1「本紙」**を追加して、**同じブック**で保存。
  - 増車: `templates_src/申請書増車1.xlsx`  
  - 減車: `templates_src/申請書減車1.xlsx`
...
| 条件 | ステータス | 表示 |
|---|---|---|
| 増車・減車を**同時**アップロード | 400 | アラート表示 → 再選択 |
| 増車・減車が**未選択** | 400 | アラート |
...
2. テンプレExcel（3つ）を **閉じた状態**で `templates_src/` に配置。
...
## 9. 実装の要点（抜粋）
- **同時アップロード禁止**と**未選択禁止**の二段ガード。
- **重複ファイル除去**（FileListの二重送信対策）。
